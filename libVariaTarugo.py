...
def simuVariaSimplesTarugo(
    ...
    for varia in vetor_varia:
        libGammaMass.mkdir(f"simulação.{area}", data=False, voltar=(varia!=vetor_varia[0]))#Voltar é false apenas para a primeira execução
        ...
        if tipoVaria == "area":
            tarugo_largura      = np.sqrt(vetor_varia)*prop,
            tarugo_altura       = np.sqrt(vetor_varia)/prop,

        elif tipoVaria == "proporção":
            tarugo_largura  = np.sqrt(area)*vetor_varia,
            tarugo_altura   = np.sqrt(area)/vetor_varia,
